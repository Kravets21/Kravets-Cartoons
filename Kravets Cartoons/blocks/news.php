<section>
        <div class="container">
			...
			if($page != 1)
			{
			    echo "<li class=\"disabled next\"><a href=\"news.php?page=$prev\"> < </a></li>";
			}
			for($i = 1; $i <= $page_numbers; $i++)
			{
			    ...
			    echo "<li$class><a href=\"news.php?page=$i\">$i</a></li>";
			}
			if($page != $page_numbers)
			{
			   echo "<li class=\"disabled next\"><a href=\"news.php?page=$next\"> > </a></li>"; 
			}
			?>
                    </ul>
                    ...
                    <?php
                    $sql = "SELECT * FROM `news` ORDER BY `id` DESC LIMIT $from, $items_on_page";
                    $query = $pdo->prepare($sql);
                    $query->execute();
